...
protected: // actions
  ...
  VIRTUAL_WITH_TESTS void
    ...
    {
      m_forwarder.onOutgoingInterest(pitEntry, outFace, interest,  mac);
    }
  /** \brief send Interest to outFace
   *  \param pitEntry PIT entry
   *  \param outFace face through which to send out the Interest
  ...
  DEPRECATED(
  void
  sendInterest(const shared_ptr<pit::Entry>& pitEntry, Face& outFace, bool wantNewNonce = false);
  )
  DEPRECATED(
    void
    sendInterest(const shared_ptr<pit::Entry>& pitEntry, Face& outFace, std::string mac, bool wantNewNonce = false);
    )
