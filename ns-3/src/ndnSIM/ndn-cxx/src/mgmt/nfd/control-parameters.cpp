...
size_t ControlParameters::wireEncode(EncodingImpl<TAG>& encoder) const {
	...
	if (this->hasMac()) {
		...
		totalLength += encoder.prependVarNumber(tlv::nfd::Mac);
	}
...
ControlParameters::wireEncode<encoding::EncoderTag>(
		EncodingImpl<encoding::EncoderTag>&) const;

template size_t
ControlParameters::wireEncode<encoding::EstimatorTag>(
		EncodingImpl<encoding::EstimatorTag>&) const;
...
void ControlParameters::wireDecode(const Block& block) {
	...
	if (this->hasUri()) {
		m_uri.assign(reinterpret_cast<const char*>(val->value()),
				val->value_size());
	}

	val = m_wire.find(tlv::nfd::LocalControlFeature);
	m_hasFields[CONTROL_PARAMETER_LOCAL_CONTROL_FEATURE] = val
			!= m_wire.elements_end();
	if (this->hasLocalControlFeature()) {
		m_localControlFeature =
	...
	if (this->hasMac()) {
		m_mac.assign(reinterpret_cast<const char*>(val->value()),
				val->value_size());
	}
	...
	val = m_wire.find(tlv::nfd::BaseTime);
	m_hasFields[CONTROL_PARAMETER_BASETIME] = val != m_wire.elements_end();
	if (this->hasBaseTime()) {
		m_baseTime = static_cast<uint64_t>(readNonNegativeInteger(*val));
	}
	...
	m_hasFields[CONTROL_PARAMETER_FACE_PERSISTENCY] = val
			!= m_wire.elements_end();
	if (this->hasFacePersistency()) {
		m_facePersistency = static_cast<FacePersistency>(readNonNegativeInteger(
				*val));
	}
}
...
operator<<(std::ostream& os, const ControlParameters& parameters) {
	...
	if (parameters.hasBaseTime()) {
		os << "BaseTime: " << parameters.getBaseTime() << ", ";
	}
